...
class InformationExporter:
    def __init__(self,args):
        ...
        self.folder_name['fig'] = f'output/figs/{problem_name}/x{args.n_var}/' \
                                  + '/' + algorithm_name + '/' + f'{args.seed}' + '/'
        self.folder_name['data'] = f'output/data/{problem_name}/x{args.n_var}/' \
                                   + '/' + algorithm_name + '/' + f'{args.seed}' + '/'
        self.folder_name['debug'] = f'output/debug/{problem_name}/x{args.n_var}/' \
                                    + '/' + algorithm_name + '/' + f'{args.seed}' + '/'
        self.folder_name['log'] = f'output/log/{problem_name}/x{args.n_var}/' \
                                  + '/' + algorithm_name + '/' + f'{args.seed}' + '/'
    ...
    @staticmethod
    def fill2D(X, Y1, Y2, c='gray', ax=None, file=None, show=False, alpha=0.3):
        if ax is None:
            _, ax = plt.subplots(1, 1)
        ax.fill_between(X.squeeze(), Y1.squeeze(), Y2.squeeze(), color=c, alpha=alpha)
        if file is not None:
            plt.grid()
    ...
    @staticmethod
    def plot2D(X, Y, c=None, ls='', marker='o', fillstyle=None, label=None, ax=None, file=None, show=False,
               show_legend=False):
        if c is None:
            c = rgb2hex(np.random.random((3,)))
        if ax is None:
            _, ax = plt.subplots(1, 1)
    ...
    @staticmethod
    def parallel_coordinate(X, c='black', bounds=None, alpha=0.3, axs=None, file=None, show=False, ):
        n_var = X.shape[1]
        if axs is None:
